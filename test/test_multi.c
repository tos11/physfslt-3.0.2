...
static void output_versions(void)
{
    ...
    PHYSFS_VERSION(&compiled);
    PHYSFS_getLinkedVersion(&linked, dv);
...
static int cmd_init(char *args)
{
    ...
    if (PHYSFS_init(args, dv))
        printf("Successful.\n");
    else
        printf("Failure. reason: %s.\n", PHYSFS_getLastError(dv));
...
static int cmd_addarchive(char *args)
{
    ...
    *ptr = '\0';
    ...
    if (PHYSFS_mount(args, NULL, appending, dv))
        printf("Successful.\n");
    else
        printf("Failure. reason: %s.\n", PHYSFS_getLastError(dv));
...
static int cmd_mount_internal(char *args, const MountType mnttype)
{
    ...
    if (mnttype == MNTTYPE_PATH)
        rc = PHYSFS_mount(args, mntpoint, appending, dv);

    else if (mnttype == MNTTYPE_HANDLE)
    {
        PHYSFS_File *f = PHYSFS_openRead(args, dv);
        if (f == NULL)
        {
            printf("PHYSFS_openRead('%s') failed. reason: %s.\n", args, PHYSFS_getLastError(dv));
            return 1;
        } /* if */

        rc = PHYSFS_mountHandle(f, args, mntpoint, appending, dv);
        if (!rc)
            PHYSFS_close(f, dv);
    } /* else if */
...
static int cmd_getmountpoint(char *args)
{
    ...
    printf("Dir [%s] is mounted at [%s].\n", args, PHYSFS_getMountPoint(args, dv));
    return 1;
} /* cmd_getmountpoint */
...
static int cmd_enumerate(char *args)
{
    ...
    rc = PHYSFS_enumerateFiles(args, dv);
...
{
    char **rc = PHYSFS_getSearchPath(dv);
...
static int cmd_getprefdir(char *args)
{
    ...
    printf("Pref dir is [%s].\n", PHYSFS_getPrefDir(org, appName, dv));
    return 1;
} /* cmd_getprefdir */
...
static int cmd_permitsyms(char *args)
{
    ...
    num = atoi(args);
    PHYSFS_permitSymbolicLinks(num, dv);
...
static int cmd_stressbuffer(char *args)
{
    ...
    else
    {
        ...
        f = PHYSFS_openWrite("test.txt", dv);
        if (f == NULL)
        ...
        else
        {
            ...
            if (!PHYSFS_setBuffer(f, num, dv))
            {
                printf("PHYSFS_setBuffer() failed: %s.\n", PHYSFS_getLastError(dv));
                PHYSFS_close(f, dv);
                PHYSFS_delete("test.txt", dv);
                return 1;
            } /* if */
            ...
            for (i = 0; i < 10; i++)
            {
                for (j = 0; j < 10000; j++)
                {
                    ...
                    if (PHYSFS_writeBytes(f, buf, left, dv) != left)
                    {
                    ...
                    if (PHYSFS_writeBytes(f, buf + left, right, dv) != right)
                    {
                        printf("PHYSFS_writeBytes() failed: %s.\n", PHYSFS_getLastError(dv));
                        PHYSFS_close(f, dv);
                        return 1;
            ...
            for (i = 0; i < 10; i++)
            {
                for (j = 0; j < 10000; j++)
                {
                    PHYSFS_uint32 right = 1 + (PHYSFS_uint32) (35.0 * rand() / (RAND_MAX + 1.0));
                    PHYSFS_uint32 left = 36 - right;
                    if (PHYSFS_readBytes(f, buf2, left, dv) != left)
                    {
                    ...
                    if (PHYSFS_readBytes(f, buf2 + left, right, dv) != right)
                    {
                        printf("PHYSFS_readBytes() failed: %s.\n", PHYSFS_getLastError(dv));
                        PHYSFS_close(f, dv);
                        return 1;
...
static int cmd_setsaneconfig(char *args)
{
    ...
    if (PHYSFS_setSaneConfig(org, appName, arcExt, inclCD, arcsFirst, dv))
        printf("Successful.\n");
    else
        printf("Failure. reason: %s.\n", PHYSFS_getLastError(dv));
...
static int cmd_isdir(char *args)
{
    ...
    rc = PHYSFS_stat(args, &statbuf, dv);
    if (rc)
...
static int cmd_cat(char *args)
{
    ...
    else
    {
        if (do_buffer_size)
        {
            if (!PHYSFS_setBuffer(f, do_buffer_size, dv))
            {
        ...
        while (1)
        {
            ...
            PHYSFS_sint64 rc;
            PHYSFS_sint64 i;
            rc = PHYSFS_readBytes(f, buffer, sizeof (buffer), dv);
...
static int cmd_cat2(char *args)
{
    ...
    if ((f1 = PHYSFS_openRead(fname1, dv)) == NULL)
        printf("failed to open '%s'. Reason: [%s].\n", fname1, PHYSFS_getLastError(dv));
    else if ((f2 = PHYSFS_openRead(fname2, dv)) == NULL)
        printf("failed to open '%s'. Reason: [%s].\n", fname2, PHYSFS_getLastError(dv));
    else
    {
        char *buffer1 = NULL;
        size_t buffer1len = 0;
        ...
        if (do_buffer_size)
        {
            if (!PHYSFS_setBuffer(f1, do_buffer_size, dv))
            {
            ...
            else if (!PHYSFS_setBuffer(f2, do_buffer_size, dv))
            {
                printf("failed to set file buffer for '%s'. Reason: [%s].\n",
                        fname2, PHYSFS_getLastError(dv));
                PHYSFS_close(f1, dv);
        ...
        do
        {
            ...
            buffer1 = ptr;
            rc = PHYSFS_readBytes(f1, buffer1 + buffer1len, readlen, dv);
            if (rc < 0)
            {
            ...
            buffer2 = ptr;
            rc = PHYSFS_readBytes(f2, buffer2 + buffer2len, readlen, dv);
            if (rc < 0)
            {
                printf("(Error condition in reading '%s'. Reason: [%s])\n\n",
                       fname2, PHYSFS_getLastError(dv));
        ...
        } while (!PHYSFS_eof(f1, dv) || !PHYSFS_eof(f2, dv), dv);
...
static int cmd_crc32(char *args)
{
    ...
    else
    {
        ...
        while ((bytesread = PHYSFS_readBytes(f, buffer, CRC32_BUFFERSIZE, dv)) > 0)
        {
            PHYSFS_uint32 i, bit;
...
static int cmd_filelength(char *args)
{
    ...
    {
        PHYSFS_sint64 len = PHYSFS_fileLength(f, dv);
        if (len == -1)
...
static int cmd_append(char *args)
{
    ...
    else
    {
        ...
        bw = strlen(WRITESTR);
        rc = PHYSFS_writeBytes(f, WRITESTR, bw, dv);
        if (rc != bw)
        {
            printf("Wrote (%d) of (%d) bytes. Reason: [%s].\n",
                   (int) rc, (int) bw, PHYSFS_getLastError(dv));
        } /* if */
        else
        {
            printf("Successful.\n");
...
{
    PHYSFS_Stat statbuf;
    if (!PHYSFS_stat(args, &statbuf, dv))
...
#else
    char envr[256];
    strcpy(envr, PHYSFS_getUserDir(dv));
...
int main(int argc, char **argv)
{
    ...
    if (!PHYSFS_deinit(dv))
        printf("PHYSFS_deinit(dv) failed!\n  reason: %s.\n", PHYSFS_getLastError(dv));
